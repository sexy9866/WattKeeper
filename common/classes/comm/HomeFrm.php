...
if(! class_exists("HomeFrm") )	{
	...
	class HomeFrm extends DB
	{
		...
		//페이지 초기화
		function initPageForMobile()
		{
			$this->rowPerPageM = $this->req["rowPerPage"] == "" ? $this->rowPerPageM : $this->req["rowPerPage"];
			$this->req["page"] = ($this->req["page"] == "" || $this->req["page"] == "0") ? "1" : $this->req["page"] ;
			$this->startNum = ($this->req["page"] - 1) * $this->rowPerPageM ;
			$this->endNum = $this->rowPerPageM ;
		}
		
		//페이지 설정
		function setPageForMobile($rownum)
		{
			$this->virtualNum	= $rownum - ($this->startNum - 0) ;
			$this->rownum = $rownum ;
			$this->endPage = ($rownum%$this->rowPerPageM) > 0 ? floor($rownum/$this->rowPerPageM)+1 : floor($rownum/$this->rowPerPageM);
			$blockNum = (floor(($this->req["page"] - 1) / $this->pagePerBlockM) * $this->pagePerBlockM) ;
			$totalBlock = floor( ($this->rownum - 1) / $this->rowPerPageM ) + 1 ;
		
			$this->startBlock = $blockNum + 1 ;
			$this->endBlock = ( $blockNum + $this->pagePerBlockM < $totalBlock ) ? $blockNum + $this->pagePerBlockM : $totalBlock ;
		}
